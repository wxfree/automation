...
class TestCustomerInterface:
    ...
    def test_customer_interface(self, args):
        ...
        resp = Util().send_request(method=method, url=Util().get_real_path(route), data=data, headers=self.get_header())
        print(resp.text)
        assert resp.json()['code'] == 200

    def get_header(self):
        return {'X-Access-Token': Util().read_yaml(TestCustomerInterface.parent_path+'/extract.yaml')['token']}
    #
    # def test_path(self):
    #     print(os.path.dirname(__file__))
